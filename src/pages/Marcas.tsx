...
import { fetchMarcas, saveMarca, deleteMarca } from "@/services/marcaService";
...
export default function Marcas() {
  ...
  const [novoMarca, setNovoMarca] = useState<Partial<Marca>>({});
  ...
  const isFormValid = useMemo(() => {
    return !!novoMarca.marca && novoMarca.marca.trim() !== '';
  }, [novoMarca.marca]);
  ...
  const marcasFiltradas = useMemo(() =>
    (marcas || []).filter(marca =>
      debouncedSearchTerm.trim().toLowerCase() === "" || marca.marca.toLowerCase().includes(debouncedSearchTerm.toLowerCase())
    ), [marcas, debouncedSearchTerm]);
  ...
  return (
    <div className="flex flex-col gap-6">
          ...
          <div className="overflow-y-auto h-full">
            {marcasFiltradas.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center p-8">
                ...
                <h3 className="text-xl font-semibold">Nenhuma Marca Encontrada</h3>
                <p className="text-muted-foreground">
                  {searchTerm ? "Nenhuma marca encontrada para sua busca." : "Clique em 'Nova Marca' para começar."}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="flex-grow space-y-4 px-6 pt-4 pb-6 max-h-[70vh] overflow-y-auto">
            <div className="space-y-2">
              ...
              <Input
                ...
                value={novoMarca.marca || ""}
                onChange={(e) => setNovoMarca({...novoMarca, marca: e.target.value})}
              />
            </div>
          </div>
      ...
      <ConfirmationDialog
        ...
        description="Esta ação não pode ser desfeita. A marca será permanentemente removida."
